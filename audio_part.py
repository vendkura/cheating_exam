...
def read_audio(stream, filename):
    ...
    seconds = 20 # Number of seconds to record at once
    filename = filename
    ...
    audio_data, _ = librosa.load(filename, sr=fs)
    reduced_noise = nr.reduce_noise(y=audio_data, sr=fs)
    
    # Save the denoised audio using scipy wavfile
    wavfile.write(filename, fs, (reduced_noise * 32767).astype(np.int16))
...
for i in range(60//10): # Number of total seconds to record/ Number of seconds per recording
